...
document.getElementById("BtnSoup").addEventListener("click", function () {
    initializeFormData().then(formData => {
        ...
        .then(function (data) {
            ...
            window.location.href ="addSoupList.html";
        })
...
var resdata = JSON.parse(localStorage.getItem('localstore-soup')) || {};
if (resdata && resdata.records) {
    ...
    document.getElementById('item-image').value = resdata.records.itemImage;
...
async function EditItem(partitionKey, rowKey) {
    try {
        ...
        localStorage.setItem('localstore-soup', restaurantdata);
        window.location.href = 'addSoup.html';
